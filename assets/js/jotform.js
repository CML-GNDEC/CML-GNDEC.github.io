            ...
            switch (type) {
                ...
                case 'radio':
                    $$("#id_" + data + ' input[type="radio"]').each(function (rad, i) {
                        if (rad.checked) {
                            if (rad.readAttribute('data-calcvalue') !== null) {
                ...
                case 'select':
                    ...
                    tempInput.select('option').each(function (option, ind) {
                        ...
                        if (option && option.selected) {
                            var current = option.readAttribute('data-calcvalue') !== null ? option.readAttribute('data-calcvalue') : optionValue(option);
                            if(numeric) {
   ...
   */
    ...
    initTestEnvForBranding21: function() {
        if (
            ...
            if (window.location.href.indexOf('branding21') === -1) return;
            var favicon = document.querySelector('link[rel="shortcut icon"]');
            window.isDarkMode = (window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches);
            if(favicon) {
                favicon.href = window.isDarkMode ? "https://cdn.jotfor.ms/assets/img/favicons/favicon-2021-dark.ico" : "https://cdn.jotfor.ms/assets/img/favicons/favicon-2021-light.ico";
            }
            var footerSelector = window.FORM_MODE === 'cardform' ? document.querySelector('.formFooter') : document.querySelector('.formFooter.f6');
            var footerLogo = document.querySelector('.formFooter-logo');
            if (footerSelector && footerLogo) {
                footerSelector.addClassName('branding21');
                footerLogo.style.height = '44px';
                footerLogo.src = 'https://cdn.jotfor.ms/assets/img/logo2021/jotform-logo-white.svg';
                footerSelector.innerHTML = footerSelector.innerHTML.replaceAll('JotForm', 'Jotform');
            }
        }
    },
    /**
    ...
    initTextboxValidation: function () {
        $$('.form-textbox[minlength]').each(function (item) {
            item.observe('blur', function () {
            ...
            item.validateTextboxMinsize = function () {
                ...
                var min_val = parseInt(item.readAttribute('minlength'));
  ...
  */
  handleSignSignatureInputs: function() {
    ...
    validateAll: function (form, scopeSelector) {
        ...
        var textboxInputs = scopeSelector.select('.form-textbox[minlength]');
